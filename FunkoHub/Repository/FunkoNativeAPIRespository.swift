//
//  FunkoNativeAPIRespository.swift
//  FunkoHub
//
...
class FunkoNativeAPIRespository: FunkoRepository {
    ...
    func getFunkoCollection(completion: @escaping ([FunkoCollection]) -> Void) {
        AF.request(Constants.url.funkoCollection, method: .get, parameters: nil, encoding: URLEncoding.default, headers: nil, interceptor: nil).response { response in
            switch response.result {
            case .success(let data):
                self.manageSuccessFunkoCollection(data: data, completion: completion)
            case .failure(_):
    ...
    func manageSuccessFunkoCollection(data: Data?, completion: @escaping ([FunkoCollection]) -> Void) {
        do {
            let decoder = JSONDecoder()
            guard let data = data else{return}
